...
export class MapModel {
  ...
  constructor() {
     ...
     * @type {Subject}
     */
    this.placemarkCoords$ = new Subject(null);
  ...
  onSelectError(successId) {
    ...
    this.placemarkCoords$.set(
      this.computeMidCountry(this.findRegion(successId))
    );
  ...
  /**
   * Вычисление средней координаты объекта геометрии на карте
   *
   * @param {Array} object Объект геометрии на карте
   * @returns {Array} Средняя координата метки
   */
  computeMidCountry(object) {
    const first = object.geometry.coordinates[0].map(([left]) => left);
    const second = object.geometry.coordinates[0].map(([, right]) => right);

    return [
      this.computeGeometryAverage(first),
      this.computeGeometryAverage(second),
    ];
  }

  /**
   * Вычисление средней координаты геометрии
   *
   * @param {Array} geometry Геометрия объекта
   * @returns {Number} Средняя координата геометрии
   */
  computeGeometryAverage(geometry) {
